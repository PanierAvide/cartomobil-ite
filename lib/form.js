...
export function getFormById(fid) {
  return forms[fid] ? addDefaults(forms[fid]) : null;
}

export function getFormObjectName(fid) {
  return forms[fid] ? addDefaults(forms[fid])['x-objectName'] : null;
}
...
export function getTagsToSendAsNotes(form, key = null) {
  let reasons = {};

  if(form["x-asNoteReason"]) {
    reasons[key] = form["x-asNoteReason"];
  }
  else if(form.properties) {
    reasons = Object.assign(
      reasons,
      ...Object.keys(form.properties).map(k => getTagsToSendAsNotes(form.properties[k], k))
    );
  }
