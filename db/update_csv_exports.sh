...
psql ${CONNEXION} -c "\copy (SELECT CASE WHEN o.fid LIKE 'n%' THEN concat('node/', substring(o.fid, 2)) WHEN o.fid LIKE 'w%' THEN concat('way/', substring(o.fid, 2)) WHEN o.fid LIKE 'r%' THEN concat('relation/', substring(o.fid, 2)) END AS osm_id, name, cat2 AS cat, brand, brand_wikidata AS wikidata, c.tags->>'vending:mask' AS has_masks, c.tags->>'vending:hydroalcoholic_gel' AS has_hydroalcoholic_gel, c.lastupdate AS last_update, country, ST_X(ST_Transform(geom, 4326))::real AS lon, ST_Y(ST_Transform(geom, 4326))::real AS lat FROM poi_cro c JOIN poi_osm o ON c.osmid = o.fid WHERE c.tags->>'vending:mask' != '' OR c.tags->>'vending:hydroalcoholic_gel' != '') TO '${OUTPUT}/poi_osm_masks_gel.csv' CSV HEADER"
